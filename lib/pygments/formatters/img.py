...
class FontManager(object):
    ...
    def _get_gae_font_path(self, name, style):
        import os
        FONT_URL = os.path.join(os.getcwd(), 'data', 'runtime', 'fonts', '{0}')

        NORMAL = {
            'Ubuntu Mono': 'UbuntuMono-R.ttf',
        }
        ITALIC = {
            'Ubuntu Mono': 'UbuntuMono-RI.ttf',
        }
        BOLD = {
            'Ubuntu Mono': 'UbuntuMono-B.ttf',
        }
        BOLDITALIC = {
            'Ubuntu Mono': 'UbuntuMono-BI.ttf',
        }
        ...
        return FONT_URL.format(BINDINGS[style][name])
    ...
    def _create_gae(self):
        for name in STYLES['NORMAL']:
            path = self._get_gae_font_path(self.font_name, name)
            if path is not None:
        ...
        for style in ('ITALIC', 'BOLD', 'BOLDITALIC'):
            for stylename in STYLES[style]:
                path = self._get_gae_font_path(self.font_name, stylename)
                if path is not None:
